...
class AnFuelpriceEnv(EnvBase):
        ...
        self.state_spec = Composite(
             {
                 ...
                 ("agents", "data"): Composite({ # Nested under "agents"
                     ...
                     "timestamp": Unbounded( # Timestamp (e.g., data index) [num_envs, 1]
    ...
    def _step(self, tensordict: TensorDictBase) -> TensorDictBase:
        ...
        next_state_tensordict = self._get_state_at(self.current_data_index) # Pass the updated data index
        ...
        output_tensordict = TensorDict({
            # Include the next observation structure from next_state_tensordict, changed 'observation' to 'data'
            ("agents", "data"): next_state_tensordict.get(("agents", "data")),
            # Include the reward
        ...
        # Debugging print to check output tensordict keys and shape before returning, changed 'observation' to 'data'
        ...
        # Also print keys and shapes of nested observation tensordict, changed 'observation' to 'data'
        if ("agents", "data") in output_tensordict.keys(include_nested=True): # Added include_nested=True
             ...
             if ("x") in nested_obs_td.keys():
                 print(f"  Nested ('agents', 'data', 'x') shape: {nested_obs_td.get('x').shape}")
                 print(f"  Nested ('agents', 'data', 'x') dtype: {nested_obs_td.get('x').dtype}")
             else:
                  print("  Nested ('agents', 'data', 'x') key NOT found.")
             # Check for timestamp key
             if "timestamp" in nested_obs_td.keys():
                  print(f"  Nested ('agents', 'data', 'timestamp') value sample: {nested_obs_td.get('timestamp')[:5]}...")
             else:
                  print("  Nested ('agents', 'data', 'timestamp') key NOT found.")

        else:
             print("  Nested ('agents', 'data') key NOT found in output tensordict.")
    ...
    def _reset(self, tensordict: Optional[TensorDictBase] = None) -> TensorDictBase:
        ...
        initial_state_tensordict = self._get_state_at(self.current_data_index) # Pass the initial data index


        # Debugging prints for initial_state_tensordict, changed 'observation' to 'data'
        ...
        if ("agents", "data") in initial_state_tensordict.keys(include_nested=True):
             nested_obs_td = initial_state_tensordict.get(("agents", "data"))
             print(f"  Nested ('agents', 'data') keys: {nested_obs_td.keys(include_nested=True)}")
             print(f"  Nested ('agents', 'data') shape: {nested_obs_td.shape}")
             if ("x") in nested_obs_td.keys():
                  print(f"  Nested ('agents', 'data', 'x') shape: {nested_obs_td.get('x').shape}")
                  print(f"  Nested ('agents', 'data', 'x') dtype: {nested_obs_td.get('x').dtype}")
             ...
             print("  Nested ('agents', 'data') key NOT found in initial_state_tensordict.")
        print("------------------------------------------------------")
        ...
        # Get the nested observation tensordict from the output of _get_state_at, changed 'observation' to 'data'
        nested_observation = initial_state_tensordict.get(("agents", "data"))

        output_tensordict = TensorDict({
            # Include the initial observation structure by setting the nested tensordict, changed 'observation' to 'data'
            ("agents", "data"): nested_observation,
        ...
        # Debugging print to check output tensordict keys before returning, changed 'observation' to 'data'
        ...
        if ("agents", "data") in output_tensordict.keys(include_nested=True): # Added include_nested=True
             ...
             # Check for timestamp key
             if "timestamp" in nested_obs_td.keys():
                  print(f"  Nested ('agents', 'data', 'timestamp') value sample: {output_tensordict.get(('agents', 'data', 'timestamp'))[0][:5]}...")
             else:
                  print("  Nested ('agents', 'data', 'timestamp') key NOT found.")
    ...
    def _batch_reward(self, data_indices: torch.Tensor, tensordict: TensorDict) -> TensorDict: # Changed actions type hint to TensorDict
        ...
        if valid_indices_flat.numel() > 0:
            ...
            # The input tensordict's batch size will match the original batch_shape of data_indices.
            # It can be [num_envs] for a single step or [num_envs, num_steps] for a rollout.
            ...
            print(f"Debug: Input tensordict batch shape to _batch_reward: {tensordict.batch_size}")
            print(f"Debug: Action tensor shape in input tensordict to _batch_reward: {actions_tensor_input.shape}")
            ...
            # The input action tensor should have shape [B, num_agents, num_action_features] for a single step
            # or [T, B, num_agents, num_action_features] for a rollout.
            # We need to flatten the batch dimensions [..., num_agents, num_action_features] -> [flat_batch_size, num_agents, num_action_features]

            if actions_tensor_input.shape[:-2] == original_batch_shape and actions_tensor_input.shape[-2:] == torch.Size([num_agents, num_action_features]):
                 actions_tensor_flat = actions_tensor_input.view(flat_batch_size, num_agents, num_action_features)
            elif actions_tensor_input.shape == torch.Size([flat_batch_size, num_agents, num_action_features]):
                 actions_tensor_flat = actions_tensor_input # Already flat
            else:
                 print(f"Error: Unexpected action tensor shape in input tensordict to _batch_reward: {actions_tensor_input.shape}. Expected batch_shape + [{num_agents}, {num_action_features}]. Using zeros for rewards.")
            ...
            if valid_indices_flat.numel() > 0 and actions_tensor_flat is not None:
                try:
                    ...
                    print(f"Debug: Action tensor shape after flattening and slicing in _batch_reward: {actions_tensor_valid_flat.shape}")
    ...
    def _get_state_at(self, data_indices: torch.Tensor) -> TensorDict:
        ...
        data_indices_clamped = torch.clamp(data_indices, 0, max_data_index)

        # Retrieve features from the combined data
        # Based on data loading, features are columns 0-12 (13 columns).
        # Node features are derived from these first 13 columns.
        # Shape [num_envs, self.num_agents]
        node_features = self.combined_data[data_indices_clamped][:, :self.num_agents]
        # Reshape node_features to include the node_feature_dim [num_envs, self.num_agents, 1]
        ...
        # Repeat the single_edge_index for the number of environments
        # The repeated edge_index will have shape [num_envs, 2, num_edges_per_graph]
        edge_index_batched = single_edge_index.unsqueeze(0).repeat(num_envs_current_batch, 1, 1)

        # Create the batch tensor for PyG
        # For num_envs environments, the batch tensor should have shape [num_envs, num_agents]
        # It maps each node in each environment's graph to its corresponding environment index.
        # Node i in environment b belongs to graph b.
        # The batch tensor should be [[0,0,...,0], [1,1,...,1], ..., [num_envs-1, ..., num_envs-1]]
        batch_tensor = torch.arange(num_envs_current_batch, device=self.device).unsqueeze(-1).repeat(1, self.num_agents) # Shape [num_envs, num_agents]


        # Add the timestamp (data index) to the observation
        # The timestamp should have shape [num_envs, 1]
        timestamps = data_indices.unsqueeze(-1) # Shape [num_envs, 1]


        # Create the TensorDict for the observation, changed 'observation' to 'data'
        observation_tensordict = TensorDict({
            "x": node_features,
            "edge_index": edge_index_batched,
            # Removed graph_attributes from the observation tensordict
            "batch": batch_tensor,
            "timestamp": timestamps, # Add timestamp to the observation
        }, batch_size=[num_envs_current_batch], device=self.device) # Set batch size to num_envs_current_batch


        # Wrap the observation tensordict under the ('agents', 'data') key, changed 'observation' to 'data'
        output_tensordict = TensorDict({
             ("agents", "data"): observation_tensordict
        }, batch_size=[num_envs_current_batch], device=self.device)


        # Debugging prints for output_tensordict, changed 'observation' to 'data'
        print("\n--- Debugging output_tensordict in _get_state_at ---")
        print(f"Output tensordict keys: {output_tensordict.keys(include_nested=True)}")
        print(f"Output tensordict shape: {output_tensordict.shape}")
        if ("agents", "data") in output_tensordict.keys(include_nested=True):
             nested_obs_td = output_tensordict.get(("agents", "data"))
             print(f"  Nested ('agents', 'data') keys: {nested_obs_td.keys(include_nested=True)}")
             print(f"  Nested ('agents', 'data') shape: {nested_obs_td.shape}")
             if ("x") in nested_obs_td.keys():
